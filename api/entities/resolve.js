...
async function resolve(type,slug){
 ...
 const x=exact(await api('/players',{search:slug.replace(/-/g,' '),page:1}),v=>v.player?.name,slug);if(!x?.player?.id)return null;return {type:'player',id:x.player.id,slug:normalize(x.player.name),name:x.player.name,photo:x.player.photo,nationality:x.player.nationality||null}
}
export default async function handler(req,res){if(req.method!=='GET')return res.status(405).json({error:'Method not allowed'});const requested=normalize(req.query?.slug),requestedType=normalize(req.query?.type),slug=aliases[requested]||requested;if(!slug)return res.status(400).json({error:'Entity slug is required'});if(requestedType&&!['league','club','player'].includes(requestedType))return res.status(400).json({error:'Unsupported entity type'});try{
 if(requestedType){const entity=await resolve(requestedType,slug);return entity?res.status(200).json(entity):res.status(404).json({error:`${requestedType} not found`})}
 // Prefer exact league matches, then exact club matches, then exact player matches. This avoids Promise timing deciding the entity type.
 for(const type of ['league','club','player']){const entity=await resolve(type,slug);if(entity)return res.status(200).json(entity)}
