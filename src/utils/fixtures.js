...
export function seasonLabel(entry, year) {
  ...
  if (!season) {
    // A year this entry doesn't describe (e.g. a club's older seasons): follow the
    // entry's own pattern, so "2026/27" is followed by "2025/26", not "2025".
    const sample = (entry?.seasons || []).find((s) => s?.start && s?.end);
    if (sample && new Date(sample.end).getFullYear() > new Date(sample.start).getFullYear()) {
...
// "W" | "D" | "L" for a finished fixture from one team's point of view, else null.
export function resultFor(fx, teamId) {
  const short = fx?.fixture?.status?.short;
  if (!isFinished(short)) return null;
  ...
  // level after extra time: a shoot-out decides via the `winner` flag
