  ...
  return h(
    ...
    [
      image
        ...
        : h("div", { className: "search-result-image", style: { background: "var(--surface-2)" } }),
  ...
  return {
    ...
    load(query) {
      ...
      run(query)
        ...
        .catch(() => {
          if (mine === seq) wrap.style.display = "none";
        });
    },
  };
}
      ...
      playerDebounce = setTimeout(() => {
        ...
        searchPlayers(q, { league: league.id, season })
          ...
          .catch((error) => {
            if (mine !== playerSeq) return;
            playerResults.innerHTML = "";
            playerResults.appendChild(emptyText(error?.message || "Couldn't load results \u2014 check your connection."));
          });
      }, 350);
    };
  }
    ...
    leagueDebounce = setTimeout(() => {
      ...
      searchLeagues(q)
        ...
        .catch((error) => {
          if (mine !== leagueSeq) return;
          leagueResults.innerHTML = "";
          leagueResults.appendChild(emptyText(error?.message || "Couldn't load results \u2014 check your connection."));
        });
    }, 350);
  });
...
function buildAllMode() {
  ...
  const emptyState = text("div", { style: { padding: "40px 18px", textAlign: "center", color: "var(--text-muted)", fontSize: "13px" } }, "Search across clubs, venues, and leagues (3+ characters).");
