...
export function VenuePage({ id, onBack }) {
  return createAsyncPage({
    title: "Venue",
    onBack,
    fetchData: () => getVenueById(id).then((r) => r[0] || null),
    ...
    renderBody: (body, v) => {
      body.appendChild(h("section", { className: "entity-hero" }, [
        ...
        metaLine([v.city, v.country], "meta-line entity-meta"),
      ]));
...
function info(label, value) {
  return value ? h("div", { className: "result-row venue-info-row" }, [
    text("span", {}, label), text("strong", {}, value),
  ]) : null;
}
