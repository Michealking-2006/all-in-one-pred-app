...
import { metaLine, entityRow, entityList, segmented, seasonPicker } from "../utils/ui.js";
...
function createLeaguePage(id, onBack) {
  ...
  const seasonBar = h("div", { hidden: true });
  const page = createTabbedPage({
    title: "League",
    onBack,
    hero: seasonBar,
    tabs: [
      { id: "overview", label: "Overview", load: () => loadOverview(id, ctx, (tab) => page.setTab(tab)) },
      { id: "standings", label: "Table", load: () => loadStandings(id, ctx) },
      { id: "matches", label: "Matches", load: () => loadMatches(id, ctx) },
      { id: "players", label: "Players", load: () => loadPlayers(id, ctx) },
    ],
  });
...
function loadMatches(id, ctx) {
  return seasonAndEntry(id, ctx).then(({ season }) => {
    ...
    function show(which) {
      list.replaceChildren(skeletonBlock());
