...
import {
  ...
  getTeamSeasons, getTeamLeaguesForSeason, getTeamSeasonFixtures,
...
import { metaLine, entityRow, entityList, seasonPicker, seasonSlot, pills, favoriteStarButton } from "../utils/ui.js";
...
import { errorNode } from "../utils/tabbedPage.js";
import { navigate } from "../router.js";

// Team statistics need a league + season. `current=true` can list cups first,
// so prefer the domestic league entry. `rows` keeps every current competition.
function resolveContext(id) {
  return getTeamLeagues(id)
    .then((rows) => {
      const entry = rows.find((r) => r.league?.type === "League") || rows[0];
      return { leagueId: entry?.league?.id || null, season: getBestSeason(entry), entry, rows };
    })
    .catch(() => ({ leagueId: null, season: null, entry: null, rows: [] }));
}

// The competitions a club played in a chosen (older) season.
function resolveSeasonContext(id, season) {
  return getTeamLeaguesForSeason(id, season).then((rows) => {
    const entry = rows.find((r) => r.league?.type === "League") || rows[0];
    return { leagueId: entry?.league?.id || null, season, entry, rows };
  });
}

export function ClubPage({ id, onBack, isFavorite, onToggleFavorite }) {
  const teamPromise = getTeamById(id).then((rows) => rows[0] || null);
  const baseCtx = resolveContext(id); // the club's current season + league
  // ctx.season === null means "the current season" (the default view). Picking
  // another season sets it and reloads every tab.
  const ctx = { season: null, competition: null };
  ...
  const page = createTabbedPage({
    ...
    hero: seasonBar.node,
    tabs: [
      { id: "overview", label: "Overview", skeleton: "overview", load: () => loadOverview(id, teamPromise, baseCtx, ctx) },
      { id: "matches", label: "Matches", skeleton: "cards", load: () => loadMatches(id, ctx) },
      { id: "squad", label: "Squad", skeleton: "people", load: () => loadSquad(id, ctx) },
      { id: "stats", label: "Stats", skeleton: "stats", load: () => loadStats(id, baseCtx, ctx) },
    ],
  });

  Promise.all([getTeamSeasons(id), baseCtx])
    .then(([rows, base]) => {
      const years = [...new Set((Array.isArray(rows) ? rows : []).map(Number).filter(Number.isInteger))].sort((a, b) => b - a);
      if (years.length < 2) return seasonBar.hide();
      const current = years.includes(base.season) ? base.season : years[0];
      seasonBar.show(seasonPicker({
        options: years.map((year) => ({ value: year, label: seasonLabel(base.entry, year) })),
        value: current,
        onChange: (value) => {
          const year = Number(value);
          ctx.season = year === base.season ? null : year;
...
// The last `n` finished matches of a fixture list (oldest to newest).
const lastPlayed = (rows, teamId, n) => rows.filter((fx) => resultFor(fx, teamId)).sort(byTime).slice(-n);

function loadOverview(id, teamPromise, contextPromise, ctx) {
  return teamPromise.then((entry) => {
    ...
    const viewingPast = ctx.season != null;
    // Time-based sections only make sense for the current season; a past season
    // gets its own form + league position instead. Manager is "now", so it is
    // left out of past seasons rather than risk showing the wrong person.
    if (viewingPast) root.append(
      asyncSection({
        title: "Season form",
        skeleton: "cards",
        load: () => getTeamSeasonFixtures(id, ctx.season),
        render: (rows) => recentFormNode(lastPlayed(rows, id, 5), id),
      }),
      asyncSection({
        title: "League position",
        skeleton: "tiles",
        load: () => resolveSeasonContext(id, ctx.season).then((c) => (c.leagueId ? getStandings(c.leagueId, ctx.season, id).then((rows) => ({ rows, ctx: c })) : null)),
      ...
      asyncSection({ title: "Recent form", skeleton: "cards", load: () => getTeamFixtures(id, { last: 5 }), render: (rows) => recentFormNode(rows, id) }),
      asyncSection({
        title: "League position",
        skeleton: "tiles",
...
function matchSections(fixtures, teamId) {
  const next = fixtures.filter((fx) => !resultFor(fx, teamId)).sort(byTime);
  const last = fixtures.filter((fx) => resultFor(fx, teamId)).sort(byTime).reverse();
  const nodes = [];
  if (next.length) { const { root: s, body } = sectionShell("Upcoming"); body.appendChild(matchList(next, teamId)); nodes.push(s); }
  if (last.length) { const { root: s, body } = sectionShell("Results"); body.appendChild(matchList(last, teamId)); nodes.push(s); }
  return nodes;
}

// A chosen past season: every match of that season, filterable by competition.
function loadSeasonMatches(id, season) {
  return getTeamSeasonFixtures(id, season).then((rows) => {
    if (!rows.length) return emptyNode("No fixtures are available for this season.");
    const competitions = [];
    rows.forEach((fx) => {
      if (fx.league?.id != null && !competitions.some((c) => c.id === String(fx.league.id))) competitions.push({ id: String(fx.league.id), label: fx.league.name });
    });
    const root = h("div", { className: "entity-page-inner" });
    const list = h("div", { className: "entity-page-inner" });
    const show = (competitionId) => {
      const visible = competitionId === "all" ? rows : rows.filter((fx) => String(fx.league?.id) === competitionId);
      list.replaceChildren(...matchSections(visible, id));
    };
    if (competitions.length > 1) root.appendChild(pills([{ id: "all", label: "All" }, ...competitions], show, "all").node);
...
function loadMatches(id, ctx) {
  if (ctx.season != null) return loadSeasonMatches(id, ctx.season);
...
function loadSquad(id, ctx) {
  return getTeamSquad(id).then((rows) => {
    ...
    const root = h("div", { className: "entity-page-inner" }, [
      ctx.season != null
        ? h("div", { className: "card info-note" }, [h("i", { "data-lucide": "info", "aria-hidden": "true" }), text("span", {}, "Squad lists are only available for the current squad. Each player's page has statistics for every season.")])
        : null,
      statTiles([["Players", players.length], ["Average age", average]]),
    ]);
...
// Statistics are per competition + season: the domestic league by default, with
// pills to switch when the club played in more than one that season.
function loadStats(id, contextPromise, ctx) {
  return contextPromise
    .then((base) => {
      const season = ctx.season ?? base.season;
      const competitions = ctx.season == null ? Promise.resolve(base.rows) : getTeamLeaguesForSeason(id, ctx.season);
      return competitions.then((rows) => ({ season, rows: (rows || []).filter((r) => r?.league?.id) }));
    })
    .then(({ season, rows }) => {
      if (!rows.length || season == null) return emptyNode("Team statistics need a league, and none was found for this club.");
      const preferred = rows.find((r) => r.league.id === ctx.competition) || rows.find((r) => r.league.type === "League") || rows[0];

      const root = h("div", { className: "entity-page-inner" });
      const host = h("div", { className: "entity-page-inner" });
      const show = (leagueId) => {
        ctx.competition = leagueId;
        host.replaceChildren(skeletonFor("stats"));
        getTeamStatistics(id, leagueId, season)
          .then((data) => { if (ctx.competition === leagueId) host.replaceChildren(renderTeamStats(data)); })
          .catch((error) => { if (ctx.competition === leagueId) host.replaceChildren(errorNode(error?.message)); });
      };
      if (rows.length > 1) {
        root.appendChild(pills(rows.map((r) => ({ id: String(r.league.id), label: r.league.name })), (value) => show(Number(value)), String(preferred.league.id)).node);
      }
      root.appendChild(host);
      show(preferred.league.id);
      return root;
    });
}
