...
export function EditProfileScreen({ onBack, avatarSrc, onChooseAvatar, authUser, onSaveName, onSignIn }) {
  const avatarSection = h("section", { className: "avatar-editor" }, [
    h("button", { className: "avatar-editor-button", onClick: onChooseAvatar, "aria-label": "Choose an avatar" }, [
      avatarSrc ? SkeletonImage({ src: avatarSrc, size: 72, radius: "50%" }) : text("span", { className: "avatar-editor-fallback" }, "S"),
    ]),
  ...
  const name = h("input", { type: "text", value: displayNameOf(authUser), maxlength: "40", autocomplete: "name" });
  const email = h("input", { type: "email", value: authUser.email || "", disabled: true });
  ...
  return h("main", { className: "screen account-form-screen" }, [
    ...
    avatarSection,
    h("section", { className: "form-card card" }, [
      h("label", { className: "form-field" }, [text("span", { className: "form-field-label" }, "Display name"), name]),
      h("label", { className: "form-field" }, [text("span", { className: "form-field-label" }, "Email"), email]),
      save,
    ]),
  ]);
}
