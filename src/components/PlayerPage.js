...
import { metaLine, entityRow, entityList, pills, seasonPicker } from "../utils/ui.js";
...
export function PlayerPage({ id, onBack }) {
  ...
  const page = createTabbedPage({
    ...
    hero: seasonBar,
    tabs: [
      { id: "overview", label: "Overview", load: () => state.profile.then(renderOverview) },
      { id: "stats", label: "Stats", load: () => state.profile.then(renderStatistics) },
      { id: "career", label: "Career", load: () => loadCareer(id) },
      { id: "trophies", label: "Trophies", load: () => loadTrophies(id) },
    ],
  });
