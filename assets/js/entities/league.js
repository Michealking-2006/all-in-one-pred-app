...
function renderFixtures(fixtures) {
  ...
  return `<div class="league-fixtures">${fixtures.map(m => { const h=m.teams?.home||{}, a=m.teams?.away||{}; const date=m.fixture?.date?new Date(m.fixture.date).toLocaleDateString(): '—'; return `<article class="league-fixture"><time>${esc(date)}</time><div><span>${esc(h.name||'Home')}</span><strong>${esc(m.goals?.home??'—')} – ${esc(m.goals?.away??'—')}</strong><span>${esc(a.name||'Away')}</span></div></article>`; }).join('')}</div>`;
}
...
async function load(root, id, season) {
  ...
  try {
    const response = await fetch(`/api/entities/league?id=${encodeURIComponent(id)}&season=${encodeURIComponent(season)}`, { signal: controller.signal, headers:{Accept:'application/json'} });
    const data = await response.json().catch(()=>({}));
    if (!response.ok) throw new Error(data.error || 'Unable to load league');
    const league=data.league?.league||{}, country=data.league?.country||{};
    ...
    if (state.controller === controller) { state.controller=null; root.removeAttribute('aria-busy'); }
  }
}
