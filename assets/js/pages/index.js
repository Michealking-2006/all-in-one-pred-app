...
function initDateUI() {
  ...
  if (!dateScroll || dateScroll.dataset.initialized) return;
  ...
  requestAnimationFrame(() => {
    dateScroll
      .querySelector(".date-card.active")
      ?.scrollIntoView({
        behavior: "smooth",
        inline: "center",
        block: "nearest"
...
function initTabsUI(signal) {
  ...
  const onClick = event => {
    const tabBtn = event.target.closest(".tab[data-tab]");
    if (!tabBtn) return;
...
function initSearchUI(signal) {
  ...
  if (!drawer || !closeBtn || !backdrop || !input || !carousel || !body) {
    return;
  }
    ...
    carousel
      ...
      .forEach(chip => {
        chip.classList.toggle(
          "active",
          chip.dataset.type === state.activeType
        );
      });
  ...
  async function runSearch() {
    ...
    const items = getItemsForActiveType(
      state.activeType
    );
  ...
  const activeChip =
    ...
    carousel.querySelector(
      '.search-filter-chip[data-type="all"]'
    );

  if (activeChip) {
    state.activeType =
      activeChip.dataset.type || "all";
    ...
    for (const country of countries) {
      ...
      for (const league of leagues) {
        flat.push({
          ...
          active:
            typeof league.active === "boolean"
              ? league.active
              : null,
          featured:
            typeof league.featured === "boolean"
              ? league.featured
              : null,
  ...
  function getItemsForActiveType(
    type
  ) {
    if (type === "all") {
      return [
  ...
  function renderResultItem(item) {
    ...
    if (item.active === true) {
      statusTags.push("Active");
    }

    if (item.featured === true) {
      statusTags.push("Featured");
    }
      ...
      >
        <div class="search-result-icon-wrap">
          ${
            iconSrc
              ...
              `
              : `
                <span class="search-result-icon-fallback">
                  ⚽
                </span>
              `
          }
        ...
        <div class="search-result-content">
          ...
          <div class="search-result-meta">
            ${
              countryHtml
                ? `<span>${countryHtml}</span>`
                : ""
            }

            ${
              divisionHtml
                ? `<span>${divisionHtml}</span>`
                : ""
            }

            ${
              statusTags.length
                ? `<span>${statusTags.join(" • ")}</span>`
                : ""
            }
          </div>
        </div>
    ...
    while (true) {
      ...
      result += `
        <span
          ...
          style="color: var(--app-primary-color); font-weight: 700;"
        >
          ${escapeHtml(
            source.slice(
              start,
