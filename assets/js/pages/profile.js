(() => {
  ...
  /*
   * The Language menu item shows the current selection.
   * languages.js persists the choice to localStorage, so this
   * just reflects it back — it works even if the person has
   * never opened the Languages page in this session.
   */
  ...
  function setSelectedLanguageLabel(root) {
    const el = root.querySelector(
      ".selected-language-profile-pg"
    );
    ...
    try {
      code =
        localStorage.getItem("scoutwave_language") ||
        "en";
    } catch (_) {
  ...
  function onLanguageChanged() {
    ...
    setSelectedLanguageLabel(state.root);
  }

  document.addEventListener(
    "scoutwave:language-changed",
    onLanguageChanged
  );
